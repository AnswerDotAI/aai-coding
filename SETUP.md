...
Outcome, kernel-centric codex, in `~/.codex/hooks.json`: PostCompact, SessionStart with matcher `compact`, and PreToolUse with matcher `mcp__clikernel__execute` each run `<venv>/bin/aai-hook codex-orientation`; UserPromptSubmit runs `<venv>/bin/aai-hook codex-prompt-submit`. Hybrid (codex or Grok Build) does not install `codex-orientation`, since it does not run the dojo; it may still install `codex-prompt-submit` in `~/.codex/hooks.json` or `~/.grok/hooks/aai.json`. codex asks the user to trust hooks on the first start after any `hooks.json` change; tell them to expect that prompt.
