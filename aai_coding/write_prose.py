...
26. Justification rider: in reference prose (docstrings, API docs, changelog entries), a fact with a benefit clause attached: "kind-sorted so a collector stays legal wherever it came from", "parses bools so flag values test correctly". The `so`/`making`/`which means` rider argues for the fact instead of stating it. Reference prose states the contract. Rationale belongs in narrative prose, and benefit clauses belong nowhere.
27. Consequence glue: any clause joined by ", so". Treat every occurrence as wrong. The appended clause is restatement, the obvious, or a link that does not hold, and on the rare occasion the consequence is real, it deserves its own sentence, where its logic must survive on its own. Delete the clause or reorganize; never keep the join ("the zmq hop must be lossless, so shedding belongs to the client queue" -> "the zmq hop must be lossless. Shedding belongs to the client queue.").
