"""Answer questions about images and PDFs via an isolated codex checker, without loading them into your own context (macOS-only): use to verify rendered output - PDFs, screenshots, captured windows.

`look` sends the images to a fresh, ephemeral gpt-5.6-luna thread whose only context is the checker
charter, your question, and the images, and returns its answer; `pdf2pngs` is the pure-Quartz page
renderer it uses, independently useful wherever a PDF needs to become images (no CLI tools, no
permissions, no size limit worries in *your* context). Needs `openai-codex` and PyObjC (workspace
installs have both); `mdhtml2docx.word.win_pic` is one producer of window captures to check."""
