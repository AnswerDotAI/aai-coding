...
Read the repo instructions and inspect its Git status and existing diff. Identify the source notebooks and the work already in progress. Preserve that work. In a dirty checkout, retain a baseline of the scoped notebooks and generated files in a temporary directory before editing. Review your changes against that baseline as well as the full Git diff. Without the baseline, you can mistake pre-existing changes for review changes. Read every cell or a summary of every cell, then read the relevant sections in full, including existing examples. Note intentional formatting before running the fixer.
...
Judge readability independently of the finding. A coherent operation can exceed the score threshold without needing an extraction or an intervening lesson. Separate helper cells can make a large implementation easier to read. Do not merge them to reduce cell counts. A valid markdown/code pair can still be unrelated to the preceding implementation. Read what it teaches before you accept the checker result.

## 3. Improve implementation before explaining around it

A long class often mixes construction, independent operations, and policy decisions. Keep its small shared setup together. Move methods with their own explanations into `@patch` cells where the notebook can develop them one at a time. Keep methods together when they depend on each other and splitting them would hide that. Do not make one cell per method as a quota.
...
Extract a helper when it names a coherent decision or transformation that a reader can understand on its own. A retry loop can separate deciding whether to stop, refresh, or wait from performing requests and sleeping. That decision can then have a short lesson using real input values. Avoid extracting the whole loop body if doing so introduces awkward sentinels, mutable state shuttling, or an elaborate result type. Shorter code is not necessarily clearer code.

Do not add a helper that only hides mock setup or moves a long block out of sight. Do not invent public APIs to satisfy the checker. Keep style refactors behavior-preserving, but fix clear, local bugs encountered during review. Being pre-existing is not a reason to leave a bug unfixed. Check the existing behavior and its callers before treating a failed new assertion as a bug. The assertion might impose a new requirement instead of exposing a defect. Validate the correction in proportion to its risk and report it separately from presentation changes. Ask before a broader redesign or a change whose intended behavior is unclear.
...
For example, `_run` creates a real container and `is_running` waits for it. They naturally share a start-and-wait lesson. You need neither a separate fake-container example for `is_running` nor a lesson for every startup helper. Introduce `_is_ready` and `make` afterward as the convenience operation that adds port readiness.
...
Do not create a fake client and patch clocks, sleeps, credentials, and network calls to demonstrate orchestration. A pure decision helper, a small real local object, or an authorized read-only integration example can be clearer. External examples need suitable authorization and execution controls. A style review is not permission to send messages, change remote data, or exercise credentials.

## 5. Put meaningful tests in lessons

Include at least one assertion of the behavior a lesson teaches, as well as its useful display. Assert the nested value, classification, calculated delay, preserved order, or documented failure. Use the notebook assertion conventions in `nbdev.skill`. Do not assert exact repr strings, incidental field order, or whole generated payloads unless callers depend on that exact representation.

Setup cells and purely illustrative displays do not need token assertions. Do not add a trivial test just to give every code cell one. One assertion is not enough if the lesson's central claim remains unchecked. Test the lesson's idea. Do not aim for a test count per cell.
...
Moving an existing passing check is not new behavior. Verify it before and after relocation rather than manufacturing a failure. Preserve its meaningful guarantees and clean up temporary files, imported example modules, and process-wide state on failure too.
...
Follow `nbdev.skill` to regenerate exports and outputs and run the changed notebooks. Run relevant pytests if implementation changed or checks moved there. Inspect external operations before executing unfamiliar notebooks. Report anything you did not run. Re-run chkstyle on the notebooks and inspect the final diff, including generated files. Preserve unrelated changes and do not commit, push, or release without authorization.

For notebooks mixing safe local examples with paid, authenticated, or mutating calls, select safe cells and their prerequisites explicitly. Save only outputs from executed cells. Report which sections ran and which outputs you did not refresh. Do not describe selected execution as a whole-notebook pass. After parallel dependency exports settle, rerun relevant dependent checks in fresh processes.

Report what became clearer, meaningful tests added or removed, deliberate exemptions, validation results, and unresolved issues. Explain the judgments you made. Do not only count warnings.
