r'''Official docs for the tool you are running in (Claude Code, codex, or Grok Build): read before answering questions about its behaviour, config, or features.
...
2. Find the page with `toc.links('<topic>')`: a few `[n] Title: description` rows are the whole display. Pick the row whose title matches the question.
   ...
   For Grok Build the published llms.txt is a concatenated dump of the xAI docs hub, not a page index. Narrow immediately: `toc.search('Grok Build')` or `toc.links('build')`. Never display the whole file. The CLI also ships the user guide at `~/.grok/docs/user-guide/`; `grok inspect` and `grok --help` are the local contracts.
...
def llms_txt(
    harness: str,  # 'claude', 'codex', or 'grok'
) -> str:
    "URL of `harness`'s official llms.txt docs index"
    return dict(claude='https://code.claude.com/docs/llms.txt',
                codex='https://developers.openai.com/llms.txt',
                grok='https://docs.x.ai/llms.txt')[harness]
