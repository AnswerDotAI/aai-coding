...
1. **Kernel-centric:** do file, shell, and Python work through clikernel, and discover tools through pyskills. This is the existing Answer.AI harness workflow and most closely matches the Claude Code setup.
...
In a fresh Claude Code or kernel-centric codex session in any workspace Python project: the bootstrap notice fires; `list_pyskills()` shows the `aai_coding.*` rows; `doc(aai_coding.coding_patterns)` renders.
