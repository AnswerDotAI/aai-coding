...
The numbers below refer to the [bracketed] markers in both passages. Where a number appears in each, the pair is the sloppy and the plain version of the same thing. Entries 17, 18, 25, and 26 are tells at scales or in genres the one-paragraph sample cannot show.
...
14. Restatement: saying the same thing twice. The original says it once, in four words. The commonest form is a lead sentence that summarizes the paragraph it starts. Everything it says reappears, with more detail, in the sentences that follow. Delete it and nothing is lost:
    ...
    The mirror form is the closing sentence that summarizes the paragraph or piece it ends ("LLMs can now use the module with far fewer tokens", ending a paragraph whose second sentence said so). It is the taught essay shape, and the strongest LLM habit of the lot. End on the last fact. The same disease occurs within a sentence ("editing needs no kernel: cards echo through the outbound queue, which is kernel-independent", where the final clause restates the opening) and between a header and its section's first line. Its listy form puts a category phrase, a colon, then a parallel list unpacking it ("it should see what you have been doing: the cells you ran, what they printed, the plots you drew, the errors you hit"). Cut whichever side of the colon says less. Ask whether deleting the phrase removes any information from the document. If not, delete it. Summaries earn their place at document scale (an abstract, a TL;DR), not per paragraph. And prefer the shorter phrasing of the same fact: "nothing new needs specifying", not "there is nothing new to learn and nothing new to specify".
15. Decoration: emoji, and decorative bold and italics. Bold and italics in the body of a paragraph should be used VERY sparingly. Avoid emojis and non-ascii unicode unless requested otherwise, e.g. "->" instead of "→".
16. Splicing: em-dash interruptions and their kin. Avoid em dashes entirely. Substituting ` - ` or ` -- ` is just as bad, being the same interrupted-clause habit with different punctuation. The clause after the splice is usually restatement or an obvious implication of the one before it. Try deleting it first (tell 14). When both clauses carry their own weight, split. Two short sentences nearly always read better than one spliced one. Phrase combiners like `:` and `;` should be rare in normal prose. A colon that introduces a list or example is fine, though see tell 14. A colon that splices two clauses is the habit to avoid, and swapping one in is no fix for an em dash.
...
25. Over-structuring: section headers imposed on a document too short to need them. Headers are navigation. A document that fits on a screen needs none. The document-scale form of tell 10's symmetry habit.

26. Consequence glue: any clause joined by ", so". Treat every occurrence as wrong. The appended clause is restatement, the obvious, or a link that does not hold, and on the rare occasion the consequence is real, it deserves its own sentence, where its logic must survive on its own. Delete the clause or reorganize ("the zmq hop must be lossless, so shedding belongs to the client queue" -> "the zmq hop must be lossless. Shedding belongs to the client queue.").
