...
RESUME_MSG = '**Post-resume: your context is exactly as it was when the app closed — doc() output and any dojo completion id remain valid — but the kernel restarted with a clean namespace (startup.py re-ran, so its imports are back).** Environmental facts in visible history (cwd, file paths, the list_pyskills() catalog) may come from another machine or an earlier date — a replayed dojo round bakes them in — so re-check them in this session rather than trusting the transcript. The doc-state record survived on disk, keyed to this conversation: doc notes fire only for functions whose docs you don\'t hold. Rebuild other session state on demand (variables, set_dlg, monkeypatches), and pass a dojo completion id from your context to dojo_start(id) before file work.'
