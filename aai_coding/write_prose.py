...
_CHARTER = ('You are a prose checker. The user message contains prose-style rules, an AUDIENCE line naming '
    'the intended readers, then the text to review under "TEXT UNDER REVIEW".\n'
    '- Report each candidate violation as: the tell name and number, and the offending span quoted verbatim. '
    'Also flag banned words, em dashes, and hard-wrapped paragraphs.\n'
    '- Judge "explaining the known" (tell 20) against the stated audience.\n'
    '- Err on the side of flagging: the caller applies judgment to your flags, so a missed tell costs more '
    'than a false positive. When a span merely resembles a tell, flag it and append "borderline".\n'
    '- Where the fix is not obvious from the flag itself, append a suggested replacement for the quoted span; '
    'never rewrite beyond that.\n'
    '- "Clean" is a valid answer when nothing matches.')
...
async def check_prose(
    ...
    "Review `text` against the rules above (this module's docstring), returning flagged spans or 'Clean'. Needs `openai-codex`."
    ...
    async with AsyncCodex() as cx:
        ...
        res = await th.run(f'# Rules\n\n{__doc__}\n\nAUDIENCE: {audience}\n\n# TEXT UNDER REVIEW\n\n{text}', effort=ReasoningEffort(effort))
