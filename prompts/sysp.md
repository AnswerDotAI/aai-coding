...
Between consecutive tool calls, plain assistant text now usually reaches the person, though the interface has dropped it before (anthropics/claude-code#75900: text emitted immediately before a thinking block) and a sentinel hook still watches for that. So narrate in text between calls as normal. Where a run of back-to-back calls is long, carrying the words in a tool the person can see is also fine, and is the safe choice.
