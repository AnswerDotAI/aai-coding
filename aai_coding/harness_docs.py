...
1. Work in `clikernel`, with the `toolslm.read_md` pyskill loaded. Fetch and parse the docs index in one step - never display the raw text: `toc = create_heading_dict(httpx.get(llms_txt(<your harness>)).text)`.
...
4. Check `len(page_txt)` in its own cell, and read the result before deciding what to display. At most 30,000 characters: parse it - `create_heading_dict(page_txt, base=<the page url, from the chosen row>)` - and display the whole `.text`; a short page read whole gives an overview sections cannot. Larger: do not display it; `search('<topic>')` the parsed page for `addr title (count)` rows and retrieve the matching sections with `at('<addr>')`, or `paths(2)` when structure itself is the question. Never display an arbitrary slice: the choice is the whole page or selected sections, nothing between. Fetch shared sections separately when an event-specific section refers to them.
